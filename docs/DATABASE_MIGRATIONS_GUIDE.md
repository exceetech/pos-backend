...
`app/main.py` was then cleaned up: all 28 functions and their `try/except` startup calls were removed. **Update (2026-08-14):** `Base.metadata.create_all(bind=engine)` has since been removed from `main.py` too — with schema fully owned by Alembic (now 47 migrations), running `create_all` alongside it was a silent drift risk (a model change without a matching migration would get created outside Alembic's history). `_seed_default_plans()` was **kept**, because it inserts/corrects *data* (subscription plan prices), not schema, and idempotent data seeding on boot is a reasonable pattern to keep.
